...
resource "null_resource" "push" {
  ...
  provisioner "local-exec" {
    ...
    command     = "${coalesce("${path.module}/scripts/push.sh")} ${var.AWSRegion} ${var.AWSProfile} ${path.module}/fastapi_demo ${aws_ecr_repository.demo-repo.repository_url} fastapi_demo-${data.external.time_stamp.result["time_stamp"]}"
    interpreter = ["bash", "-c"]
  }
}